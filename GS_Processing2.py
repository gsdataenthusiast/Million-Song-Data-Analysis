...
# loading the data from main dataset
main = (
...
# loading the metadata file from main
main_meta = (
	spark.read.format("com.databricks.spark.csv") \
	.option("header", "true") \
	.option("inferSchema", "true") \
...
# unique songs from the metadata file
distinct = main_meta.agg(countDistinct("song_id")).alias('unique_songs')
distinct.show()
...
##Loading Mismatch Manually accepted to dataframe
...
##loading mismatches data into dataframe
...
##to retrieve the songs in tasteprofile data that are not mismatched
...
##define function to automate StructType creation
...
#msd-jmir-area-of-moments-all-v1.0
#StructType(List(StructField(feature_0000,DoubleTyp ... Type,true),StructField(track_id,StringType,true)))
#msd-jmir-lpc-all-v1.0
#StructType(List(StructField(feature_0000,DoubleTyp ... Type,true),StructField(track_id,StringType,true)))
#msd-jmir-methods-of-moments-all-v1.0
#StructType(List(StructField(feature_0000,DoubleTyp ... Type,true),StructField(track_id,StringType,true)))
